...
void partition_right_orbits(linear L){
   size_t i, x;
   for (i = 1; i < L.N; i++) {
      if(orbits.x[i]>=i){
         x = L.y[i];
...
void partition_left_orbits(linear L){
   size_t i, x;
   for (i = 1; i < L.N; i++) {
      if(orbits.y[i]>=i){
         x = L.y[i];
...
void guess(vbf_tt F, vbf_tt G, triplicate Ft, triplicate Gt, linear L1, linear L2, vbf_tt_entry *fgs, vbf_tt_entry *xgs, unsigned char px, unsigned char cfg){
   ...
   if (i == F.vbf_tt_number_of_entries-1) {// || px == (F.vbf_tt_dimension/2)+1) { // one of these conditions is enough
      partition_right_orbits(L2);
      //partition_left_orbits(L1);
      return;
   }
...
int main(int argc, char *argv[]){
   ...
   orbits.N = F.vbf_tt_number_of_entries;
   orbits.x = (vbf_tt_entry *) malloc (F.vbf_tt_number_of_entries * sizeof (vbf_tt_entry)); // right orbits L2 
   orbits.y = (vbf_tt_entry *) malloc (F.vbf_tt_number_of_entries * sizeof (vbf_tt_entry)); // left orbits L1 

   for(i=0;i<orbits.N;i++){
      orbits.x[i]=i;
      //orbits.y[i]=i;
   }
   ...
   print_linear(orbits);
   delete_linear_from_memory(orbits);
